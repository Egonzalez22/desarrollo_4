...
class AccountMove(models.Model):
        ...
        for i in self:
            if i.partner_id:
                if not i.partner_id.vat:
                    ...
                    # Si el partner ya tiene carado el tipo de identificacion, se toma ese valor
                    if i.partner_id.res90_tipo_identificacion:
                        i.res90_tipo_identificacion = i.partner_id.res90_tipo_identificacion
                    else:
                        pattern = "^[0-9]+-[0-9]$"
                ...
                i.excluir_res90 = i.partner_id.proveedor_facturador_electronico
    ...
    def get_monto_exento(self):
        if self.journal_id.res90_tipo_comprobante == '101':  # Código Autofactura
            return 0  # Si es una autofactura, el monto exento debe ser cero
        else:
            monto0 = sum(self.invoice_line_ids.filtered(lambda x: not x.tax_ids or 0 in x.tax_ids.mapped('amount')).
                         mapped('price_total'))
            if self.currency_id != self.env.company.currency_id:
                balance = abs(sum(self.invoice_line_ids.filtered(lambda x: x.currency_id == self.currency_id).
                                  mapped('balance')))
                amount_currency = abs(sum(self.invoice_line_ids.filtered(lambda x: x.currency_id == self.currency_id).
