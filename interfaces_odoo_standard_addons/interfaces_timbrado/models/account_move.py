...
""")
    ...
    def obtener_timbrado(self):
        tipo_documento = self.move_type
        return self.journal_id.timbrados_ids.filtered(lambda x: x.active and x.tipo_documento == tipo_documento)

    def validar_timbrado(self):
        if self.move_type in ['out_invoice', 'out_refund'] and self.name and self.name != '/':
            timbrado = self.obtener_timbrado()
            if len(timbrado) > 1:
            ...
            if timbrado:
                ...
                if nro_pto_expedicion != timbrado.nro_punto_expedicion:
                    raise exceptions.ValidationError('El numero de punto de expedicion no coincide con el del timbrado activo')
                if nro_establecimiento != timbrado.nro_establecimiento:
                    raise exceptions.ValidationError('El numero de establecimiento no coincide con el del timbrado activo')
                ...
                if fecha < timbrado.inicio_vigencia:
                    raise exceptions.ValidationError('La fecha de la factura no puede ser menor a la fecha de fin de vigencia del timbrado')
            else:
