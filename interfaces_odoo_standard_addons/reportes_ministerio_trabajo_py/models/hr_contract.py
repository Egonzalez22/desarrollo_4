...
class HrContract(models.Model):
    ...
    def get_real_wage(self, request_date=fields.Date.today()):
        ...
        result = {
            ...
            'real_wage_words': self.env['interfaces_tools.tools'].numero_a_letra(int(salario)),
            'real_wage_monthly': salario_mensual,
            'real_wage_monthly_string': "{:,.0f}".format(salario_mensual).replace(',', '.'),
            'real_wage_monthly_words': self.env['interfaces_tools.tools'].numero_a_letra(int(salario_mensual)),
        }
        return result
