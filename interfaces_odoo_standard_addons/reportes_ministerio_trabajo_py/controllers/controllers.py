...
class ReportesEyOSyJZIP(http.Controller):
    ...
    def download_empleados_obreros_report(self, kw_aux=False, **kw):
        ...
        if request.env.user.has_group('hr.group_hr_manager'):
            ...
            mtess_reports_tmp_folder_mode = request.env['ir.config_parameter'].sudo().get_param(
                'mtess_reports_tmp_folder_mode') == 'True'
            if mtess_reports_tmp_folder_mode:
                os.chdir('/tmp')
                ...
                for company in companies:
                    ...
                    for employee in nominas_periodo.contract_id.employee_id:
                        ...
                        if employee.address_home_id.state_id:
                            contenido_txt+=employee.address_home_id.state_id.code+';'
                        else:
                            contenido_txt += '1;'
                        # 16
                        if employee.address_home_id.city_id.district_id:
                            contenido_txt+=employee.address_home_id.city_id.district_id.code+';'
                        else:
                            contenido_txt += '1;'
                        # 17
                        contenido_txt += ';'
                        ...
                        celular_prefijo = ''
                        celular_numero = ''
                        celular = employee.phone or employee.mobile_phone
                        if celular:
                            celular = celular.replace(' ', '').replace('-', '')
                            if '-' in celular:
                                celular = celular.split('-')
                                celular_prefijo = celular[0]
                                celular_numero = ''.join(celular[1:])
                            else:
                                celular_prefijo = celular[:4]
                                celular_numero = celular[4:]

                        contenido_txt += celular_prefijo + ';'
                        # 22
                        contenido_txt += celular_numero + ';'
                        # 23
                        contenido_txt += '1'
                    # Cabecera
