...
class WizardCalculoRenuncia(models.TransientModel):
    ...
    def onChangeTipoRenuncia(self):
        if self.tipo_renuncia in ['fallecimiento', 'jubilacion', 'termino contrato']:
            self.dias_preaviso_recibido = self.dias_preaviso_correspondido
            if self.tipo_renuncia not in ['termino contrato']:
            ...
            if self.contract_id:
                ...
                if self.tipo_empleado in ['30', '24']:
                    ...
                    for ultimo_salario in ultimos_salarios:
                        if (ultimo_salario.date_from.year, ultimo_salario.date_from.month) not in meses_ultimos_salarios:
                            meses_ultimos_salarios.append((ultimo_salario.date_from.year, ultimo_salario.date_from.month))
                    ...
                    for i in range(1, 7):
                        if len(meses_ultimos_salarios) >= i:
                            self['salario_mes_' + str(i)] = sum(
                                ultimos_salarios.filtered(
                                    lambda x:
                                    x.date_from.year == meses_ultimos_salarios[i - 1][0] and
                                    x.date_from.month == meses_ultimos_salarios[i - 1][1]
                                ).mapped('real_wage')
                            )
    ...
    def let_the_purge_begin(self):
        ...
        elif self.tipo_renuncia in ['termino contrato']:
            name = 'Término de Contrato'
            motivo = 'Renuncia - Término de Contrato'
