...
class AccountMove(models.Model):
    ...
    def write(self, vals):
        ...
        for record in self:  
            if record.es_factura_exterior and record.invoice_line_ids.tax_ids and any(record.invoice_line_ids.tax_ids.mapped('amount')):
                raise exceptions.ValidationError(
                    'Una Factura Exterior (Importación) solo debe tener Exentas como impuestos'
                )
        return res
