    ...
    def compute_secondary_values(self):
        ...
        if secondary_currency_id == self.currency_id:
            ...
            if self.move_id.freeze_currency_rate:
                ...
                secundary_rate_ids = secondary_currency_id.rate_ids.filtered(lambda x: x.name <= self.date)
                
                # Se obtiene el inverse_company_rate si hay tipo_cambio definido
                if secundary_rate_ids:
                    tipo_cambio = round(secundary_rate_ids[0].inverse_company_rate,2)
