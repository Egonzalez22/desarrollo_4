...
class HrPayslipRunReportXLSX(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, payslip_run_ids):
        ...
        def simpleWrite(to_write, format=None):
            ...
            if isinstance(to_write, (int, float)):
                sumatorias[position_x] = sumatorias.get(position_x, 0) + to_write
        ...
        addSalto()
        simpleWrite('TOTAL', bold)
        addRight()
        for col in range(1, total_columns + 1):
            simpleWrite(sumatorias.get(col, ''), numerico_total)
            addRight()
