...
class AccountMoveLine(models.Model):
    ...
    def gap_get_odoo_rate(self, company_currency, recon_currency, vals, other_line=None):
        ...
        def is_payment(vals):
            return vals.get("is_payment") or (
                vals.get("record") and bool(vals["record"].move_id.payment_id or vals["record"].move_id.statement_line_id)
            )
        ...
        if (
            ...
            move_id = self.env[self.env.context.get("active_model")].browse(self.env.context.get("active_ids"))
            if move_id.currency_id != move_id.company_currency_id:
                return 1 / move_id.currency_rate

        if other_line.get("record") and other_line["record"].move_id:
            if other_line["record"].move_id.move_type == "out_invoice":
                return other_line["record"].move_id.currency_id._get_conversion_rate_tipo_cambio_comprador(
                    vals["record"].move_id.currency_id,
                    other_line["record"].move_id.currency_id,
                    other_line["record"].move_id.company_id,
                    vals["record"].move_id.date,
                )
            if other_line["record"].move_id.move_type == "in_invoice":
                return other_line["record"].move_id.currency_id._get_conversion_rate_tipo_cambio_vendedor(
                    vals["record"].move_id.currency_id,
                    other_line["record"].move_id.currency_id,
                    other_line["record"].move_id.company_id,
                    vals["record"].move_id.date,
    ...
    def _prepare_reconciliation_single_partial(self, debit_vals, credit_vals):
        ...
        def get_odoo_rate(vals, other_line=None):
            return self.gap_get_odoo_rate(company_currency, recon_currency, vals, other_line)
        ...
        is_rec_pay_account = debit_vals.get("record") and debit_vals["record"].account_type in ("asset_receivable", "liability_payable")
