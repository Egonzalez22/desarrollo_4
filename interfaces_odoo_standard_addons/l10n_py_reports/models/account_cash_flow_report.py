...
class Res49CashflowCustomHandler(models.AbstractModel):
    _name = 'l10npy_reports.cashflow.report.handler'
    ...
    def _get_tags_ids(self):
        ...
        return {
            'operativas_ventas':self.env.ref('l10n_py_reports.tag_FLEF_OPERATIVAS').id,
            'operativas_pago_proveedores_locales':self.env.ref('l10n_py_reports.tag_FLEF_OPERATIVAS_PAGO_PROVEEDORES_LOCALES').id,
            'operativas_pago_proveedores_exterior':self.env.ref('l10n_py_reports.tag_FLEF_OPERATIVAS_PAGO_PROVEEDORES_EXTERIOR').id,
            'operativas_pago_empleados':self.env.ref('l10n_py_reports.tag_FLEF_OPERATIVAS_PAGO_EMPLEADOS').id,
            'operativas_pago_otras_actividades':self.env.ref('l10n_py_reports.tag_FLEF_OPERATIVAS_PAGO_OTRAS_ACTIVIDADES').id,
            'operativas_pago_impuestos':self.env.ref('l10n_py_reports.tag_FLEF_OPERATIVAS_PAGO_IMPUESTOS').id,
            'inversiones_temporarias':self.env.ref('l10n_py_reports.tag_FLEF_INVERSION').id,
            'inversiones_largo_plazo':self.env.ref('l10n_py_reports.tag_FLEF_INVERSIONES_LARGO_PLAZO').id,
            'inversiones_propiedad':self.env.ref('l10n_py_reports.tag_FLEF_INVERSIONES_PROPIEDAD').id,
            'financiamiento_aporte_capital':self.env.ref('l10n_py_reports.tag_FLEF_FINANCIAMIENTO').id,
            'finaciamiento_prestamos':self.env.ref('l10n_py_reports.tag_FLEF_FINANCIAMIENTO_PRESTAMOS').id,
            'financiamiento_dividendos':self.env.ref('l10n_py_reports.tag_FLEF_FINANCIAMIENTO_DIVIDENDOS').id,
            'financiamiento_intereses':self.env.ref('l10n_py_reports.tag_FLEF_FINANCIAMIENTO_INTERESES').id,
            'diferencia_cambio':self.env.ref('l10n_py_reports.tag_FLEF_DIFERENCIA_CAMBIO').id,
        }
    ...
    def _dispatch_aml_data(self, tags_ids, aml_data, layout_data, report_data):
        ...
        #     self._add_report_data('operativas_ventas', aml_data, layout_data, report_data)
        # elif aml_data['account_account_type'] == 'liability_payable':
        #     self._add_report_data('operativas_pago_proveedores_locales', aml_data, layout_data, report_data)
        # else:
        if aml_data['account_tag_id'] == tags_ids['operativas_ventas']:
            self._add_report_data('operativas_ventas', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['operativas_pago_proveedores_locales']:
            self._add_report_data('operativas_pago_proveedores_locales', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['operativas_pago_proveedores_exterior']:
            self._add_report_data('operativas_pago_proveedores_exterior', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['operativas_pago_empleados']:
            self._add_report_data('operativas_pago_empleados', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['operativas_pago_otras_actividades']:
            self._add_report_data('operativas_pago_otras_actividades', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['operativas_pago_impuestos']:
            self._add_report_data('operativas_pago_impuestos', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['inversiones_temporarias']:
            self._add_report_data('inversiones_temporarias', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['inversiones_largo_plazo']:
            self._add_report_data('inversiones_largo_plazo', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['inversiones_propiedad']:
            self._add_report_data('inversiones_propiedad', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['financiamiento_aporte_capital']:
            self._add_report_data('financiamiento_aporte_capital', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['finaciamiento_prestamos']:
            self._add_report_data('finaciamiento_prestamos', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['financiamiento_dividendos']:
            self._add_report_data('financiamiento_dividendos', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['financiamiento_intereses']:
            self._add_report_data('financiamiento_intereses', aml_data, layout_data, report_data)
        elif aml_data['account_tag_id'] == tags_ids['diferencia_cambio']:
            self._add_report_data('diferencia_cambio', aml_data, layout_data, report_data)
        else:
            self._add_report_data('unclassified_activities', aml_data, layout_data, report_data)
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            queries.append(f'''
                SELECT
                    ...
                    SUM(ROUND(account_move_line.balance)) AS balance
                FROM {tables}
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            queries.append(f'''
                ...
                SELECT
                    ...
                    SUM(ROUND(account_partial_reconcile.amount)) AS balance
    ...
    def _get_reconciled_moves(self, report, options, currency_table_query, payment_account_ids, payment_move_ids, cash_flow_tag_ids):
        ...
        for aml_data in self._cr.dictfetchall():
            ...
            reconciled_percentage_per_move[aml_data['column_group_key']][aml_data['move_id']][aml_data['account_id']][0] += aml_data['balance'] or 0
        ...
        for aml_data in self._cr.dictfetchall():
            if aml_data['account_id'] in reconciled_percentage_per_move[aml_data['column_group_key']][aml_data['move_id']]:
                reconciled_percentage_per_move[aml_data['column_group_key']][aml_data['move_id']][aml_data['account_id']][1] += aml_data['balance'] or 0
    ...
    #             'exchange_activities': {'name': _('Cash flows from exchange activities'), 'level': 2, 'parent_line_id': 'net_increase'},
    #                 'exchange_activities_cash_in': {'name': _('Cash in'), 'level': 3, 'parent_line_id': 'exchange_activities'},
    #                 'exchange_activities_cash_out': {'name': _('Cash out'), 'level': 3, 'parent_line_id': 'exchange_activities'},
    ...
    # #     # Indentation of the following dict reflects the structure of the report.
    #     return {
    #             'operating_activities': {'name': _('FLUJO DE EFECTIVO POR ACTIVIDADES OPERATIVAS'), 'level': 2, 'parent_line_id': 'net_increase'},
    #                 'advance_payments_customer': {'name': _('Anticipos recibidos de clientes'), 'level': 3, 'parent_line_id': 'operating_activities'},
    #                 'received_operating_activities': {'name': _('Efectivo recibido por actividades operativas'), 'level': 3, 'parent_line_id': 'operating_activities'},
    #                 'advance_payments_suppliers': {'name': _('Anticipos realizados a proveedores'), 'level': 3, 'parent_line_id': 'operating_activities'},
    #                 'paid_operating_activities': {'name': _('Efectivo gastado por actividades operativas'), 'level': 3, 'parent_line_id': 'operating_activities'},
    #             'investing_activities': {'name': _('FLUJO DE EFECTIVO POR ACTIVIDADES DE INVERSION'), 'level': 2, 'parent_line_id': 'net_increase'},
    #                 'investing_activities_cash_in': {'name': _('Efectivo recibido'), 'level': 3, 'parent_line_id': 'investing_activities'},
    #                 'investing_activities_cash_out': {'name': _('Efectivo gastado'), 'level': 3, 'parent_line_id': 'investing_activities'},
    #             'financing_activities': {'name': _('FLUJO DE EFECTIVO POR ACTIVIDADES DE FINANCIAMIENTO'), 'level': 2, 'parent_line_id': 'net_increase'},
    #                 'financing_activities_cash_in': {'name': _('Efectivo recibido'), 'level': 3, 'parent_line_id': 'financing_activities'},
    #                 'financing_activities_cash_out': {'name': _('Efectivo gastado'), 'level': 3, 'parent_line_id': 'financing_activities'},
    #             'exchange_activities': {'name': _('EFECTO DE LAS GANANCIAS O PERDIDAS POR DIFERENCIA DE TIPO DE CAMBIO'), 'level': 2, 'parent_line_id': 'net_increase'},
    #                 'exchange_activities_cash_in': {'name': _('Efectivo recibido'), 'level': 3, 'parent_line_id': 'exchange_activities'},
    #                 'exchange_activities_cash_out': {'name': _('Efectivo gastado'), 'level': 3, 'parent_line_id': 'exchange_activities'},
    #             'unclassified_activities': {'name': _('Cash flows from unclassified activities'), 'level': 2, 'parent_line_id': 'net_increase'},
    #                 'unclassified_activities_cash_in': {'name': _('Efectivo recibido'), 'level': 3, 'parent_line_id': 'unclassified_activities'},
    #                 'unclassified_activities_cash_out': {'name': _('Efectivo gastado'), 'level': 3, 'parent_line_id': 'unclassified_activities'},
    #         'net_increase': {'name': _('AUMENTO/DISMINUCIÓN NETO/A DE EFECTIVOS Y SUS EQUIVALENTES'), 'level': 0},
                
                
    #         'opening_balance': {'name': _('EFECTIVO Y SUS EQUIVALENTES AL COMIENZO DEL PERIODO'), 'level': 0},
    #         'closing_balance': {'name': _('EFECTIVO Y SUS EQUIVALENTES AL CIERRE DEL PERIODO'), 'level': 0},
    #     }
    def _get_layout_data(self):
    #     # Indentation of the following dict reflects the structure of the report.
        return {
                'operating_activities': {'name': _('FLUJO DE EFECTIVO POR ACTIVIDADES OPERATIVAS'), 'level': 2, 'parent_line_id': 'net_increase'},
                    'operativas_ventas': {'name': _('Ventas netas (Cobro neto)'), 'level': 3, 'parent_line_id': 'operating_activities'},
                    'operativas_pago_proveedores_locales': {'name': _('Pago a proveedores locales (Pago neto)'), 'level': 3, 'parent_line_id': 'operating_activities'},
                    'operativas_pago_proveedores_exterior': {'name': _('Pago a proveedores del exterior (Pago neto)'), 'level': 3, 'parent_line_id': 'operating_activities'},
                    'operativas_pago_empleados': {'name': _('Efectivo pagado a empleados'), 'level': 3, 'parent_line_id': 'operating_activities'},
                    'operativas_pago_otras_actividades': {'name': _('Efectivo generado (Usado) en otras actividades'), 'level': 3, 'parent_line_id': 'operating_activities'},
                    'operativas_pago_impuestos': {'name': _('Pago de impuestos'), 'level': 3, 'parent_line_id': 'operating_activities'},
                'investing_activities': {'name': _('FLUJO DE EFECTIVO POR ACTIVIDADES DE INVERSION'), 'level': 2, 'parent_line_id': 'net_increase'},
                    'inversiones_temporarias': {'name': 'Aumento / Disminución Neto/a de actividades temporarias', 'level': 3, 'parent_line_id': 'investing_activities'},
                    'inversiones_largo_plazo': {'name': 'Aumento / Disminución Neto/a de actividades a largo plazo', 'level': 3, 'parent_line_id': 'investing_activities'},
                    'inversiones_propiedad': {'name': 'Aumento / Disminución Neto/a de Propiedad, Planta y Equipo', 'level': 3, 'parent_line_id': 'investing_activities'},
                'financing_activities': {'name': _('FLUJO DE EFECTIVO POR ACTIVIDADES DE FINANCIAMIENTO'), 'level': 2, 'parent_line_id': 'net_increase'},
                    'financiamiento_aporte_capital': {'name': 'Aporte de Capital', 'level': 3, 'parent_line_id': 'financing_activities'},
                    'finaciamiento_prestamos': {'name': 'Aumento / Disminución Neto de Préstamos', 'level': 3, 'parent_line_id': 'financing_activities'},
                    'financiamiento_dividendos': {'name': 'Dividendos pagados', 'level': 3, 'parent_line_id': 'financing_activities'},
                    'financiamiento_intereses': {'name': 'Aumento / Disminución neto de Intereses', 'level': 3, 'parent_line_id': 'financing_activities'},
                'exchange_activities': {'name': _('EFECTO DE LAS GANANCIAS O PERDIDAS POR DIFERENCIA DE TIPO DE CAMBIO'), 'level': 2, 'parent_line_id': 'net_increase'},
                    'diferencia_cambio': {'name': 'Diferencias de cambio', 'level': 3, 'parent_line_id': 'exchange_activities'},
                'unclassified_activities': {'name': _('Flujo de efectivo de actividades no clasificadas'), 'level': 2, 'parent_line_id': 'net_increase'},
                    # 'unclassified_activities_cash_in_out': {'name': 'Movimientos', 'level': 3, 'parent_line_id': 'unclassified_activities'},
            'net_increase': {'name': _('AUMENTO/DISMINUCIÓN NETO/A DE EFECTIVOS Y SUS EQUIVALENTES'), 'level': 0},
