...
class AssetReportCustomHandler(models.AbstractModel):
    ...
    def _generate_report_lines_without_grouping(self, report, options, prefix_to_match=None, parent_id=None, forced_account_id=None):
        ...
        # INTERFACES: Nuevas columnas
        column_names = [
            'assets_date_from', 'assets_plus', 'assets_minus', 'assets_date_to', 'depre_date_from',
            'depre_plus', 'depre_minus', 'depre_date_to', 'balance', 'valor_prueba', 'coeficiente_revaluo',
            'valor_residual_porc', 'valor_residual_fiscal', 'anos_vida_util_fiscal', 'anos_vida_util_fiscal_restante',
            'valor_fiscal_neto_anterior', 'valor_fiscal_revaluado', 'cuota_fisca_depreciacion_anual', 'valor_residual_contable',
            'anos_vida_util_contable', 'anos_vida_util_contable_restante', 'valor_contable_neto_anterior', 'valor_contable_revaluado',
            'cuota_contable_depreciacion_anual', 'cuota_contable_depreciacion_anual_no_deducible',
        ]
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        # INTERFACES: Eliminamos la sobreescritura de los nombres de columnas
    ...
    def _query_lines(self, options, prefix_to_match=None, forced_account_id=None):
        ...
        for al in parent_lines:
            ...
            if al['asset_method'] == 'linear' and al['asset_method_number'] == 0:  # some assets might have 0 depreciations because they dont lose value
            ...
            # INTERFACES: Nuevos Valores
            valor_residual_porc = al["valor_residual_porc"] or 0
            current_depreciation_value = al["current_depreciation_value"] or 0
            if current_depreciation_value > 0:
                es_mensual = al["asset_method_period"] == '1'
                if es_mensual:
                    current_depreciation_value = current_depreciation_value * 12

            anos_vida_util_restante =  al["asset_acquisition_date"].year + years - datetime.now().year
            valor_neto_ejercicio_anterior = asset_closing - depreciation_closing + current_depreciation_value
            valor_residual =  valor_residual_porc * asset_opening / 100
            ...
            columns_by_expr_label = {
                ...
                "coeficiente_revaluo": 0,
                "valor_residual_porc": valor_residual_porc,
                "valor_residual_fiscal": valor_residual,
                "anos_vida_util_fiscal": years, 
                "anos_vida_util_fiscal_restante": anos_vida_util_restante, 
                "valor_fiscal_neto_anterior": valor_neto_ejercicio_anterior,
                "valor_fiscal_revaluado": valor_neto_ejercicio_anterior - valor_residual,
                "cuota_fisca_depreciacion_anual": current_depreciation_value,
                "valor_residual_contable": valor_residual,
                "anos_vida_util_contable": years, 
                "anos_vida_util_contable_restante": anos_vida_util_restante, 
                "valor_contable_neto_anterior": valor_neto_ejercicio_anterior,
                "valor_contable_revaluado": valor_neto_ejercicio_anterior - valor_residual,
                "cuota_contable_depreciacion_anual": current_depreciation_value,
                "cuota_contable_depreciacion_anual_no_deducible": 0,
            }
    ...
    def _query_values(self, options, prefix_to_match=None, forced_account_id=None):
        ...
        sql = f"""
            SELECT asset.id AS asset_id,
                   ...
                   asset.valor_residual_porc AS valor_residual_porc,
                   account.code AS account_code,
                   account.name AS account_name,
                   account.id AS account_id,
                   account.company_id AS company_id,
                   (
                        SELECT move.depreciation_value
                        FROM account_move AS move
                        WHERE move.asset_id = asset.id and state = 'posted'
                        ORDER BY move.date DESC
                        limit 1
                    ) AS current_depreciation_value,
