...
class Wizard(models.TransientModel):
    ...
    def button_imprimir(self):
        ...
        return self.env.ref('reporte_libro_diario.reporte_diario_action').report_action(self)
...
class ReporteLibroDiario(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data):
        wizard = self.env['reporte_libro_diario.wizard'].browse(docids)
        company_obj = wizard.compania
        moves = self.env['account.move'].sudo().search(
            [
                ('date', '>=', wizard.fecha_inicio),
                ('date', '<=', wizard.fecha_fin),
                ('state', '=', 'posted'),
                ('company_id', '=', wizard.compania.id)
            ]
        )
        if moves:
            moves = moves.sorted(key=lambda x: (x.nro_asiento))
        docargs = {
            'docs': moves,
...
class ReporteLibroDiarioXls(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, datas):
        ...
        for move in moves:
            breakAndWrite("Asiento: %s" % move.nro_asiento, bold)
