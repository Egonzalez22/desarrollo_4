...
def to_word(numero):
    ...
    if numero_entero < 0:
        return 'menos %s' % to_word(abs(numero))
    letras_decimal = ''
    parte_decimal = int(round((abs(numero) - abs(numero_entero)) * 100))
    if parte_decimal > 9:
        letras_decimal = 'con %s' % to_word(parte_decimal)
    elif parte_decimal > 0:
        letras_decimal = 'con cero %s' % to_word(parte_decimal)
    if (numero_entero <= 99):
...
def numero_a_moneda(numero):
    ...
    letras = to_word(numero_entero)
    letras = letras.replace('uno', 'un')
    letras_decimal = 'con %s %s' % (to_word(parte_decimal).replace('uno', 'un'), centimos)
...
def leer_decenas(numero):
    ...
    if numero <= 19:
        ...
        resultado = 'veinti%s' % UNIDADES[unidad] if unidad > 0 else 'veinte'
    else:
...
def leer_centenas(numero):
    ...
    if numero == 0:
        ...
        # resultado = CIENTOS[centena] if decena > 0 else 'cien'
        resultado = 'cien' if numero == 100 else CIENTOS[centena]
        if decena > 0:
